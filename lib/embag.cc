...
{
    auto fields = std::make_unique<std::unordered_map<std::string, std::string>>();
...
bool Bag::readRecords(boost::iostreams::stream<T> &stream)
{
    ...
    for (size_t i = 0; i < chunk_count; i++) {
        ...
        // TODO: The chunk infos are not necessarily Revisit this logic if seeking back and forth across the file causes
        // a slowdown
